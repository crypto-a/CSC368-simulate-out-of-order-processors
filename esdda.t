...
void handle_tlb_fault(asid_t asid, pagetable_t *pt, vaddr_t vaddr, char type, bool write)
{
	...
	tlb_index_t existing_idx = tlbp(asid, vpn);

	if (existing_idx != TLB_PROBE_NOTFOUND && write) {
		// wrtie fault not tlb miss
		write_fault_count++;
		ref_count++;

		tlb_entry_t entry;
		tlbr(existing_idx, &entry);

		frame_t *frame = frame_from_number(entry.fields.pfn);
		ptrarray_slice_t ptes = get_referring_ptes(frame);
		pt_entry_t *pte = NULL;
		for (size_t i = 0; i < ptes.len; i++) {
			pt_entry_t *curr_pte = (pt_entry_t *)ptes.ptr[i];
			if (curr_pte->vpn == vpn && curr_pte->asid == asid) {
				pte = curr_pte;
				break;
			}
		}
		...
		if (pte && !pte->writable) {
			// CoW cas
			char effective_type = 'M';
			pte = page_walk(pt, vaddr, effective_type);
			...
			tlb_entry_t new_ent = (tlb_entry_t){
				.fields = {
		...
		entry.fields.dirty = true;
		tlbwi(existing_idx, &entry);
		ref_func(entry.fields.pfn);
	...
	if (existing_idx != TLB_PROBE_NOTFOUND) {
		tlbwi(existing_idx, &tlb_ent);
	} else {
		tlbwr(&tlb_ent);
	}
}
